...
class AwardScraper:
    ...
    async def scrape_list_page(
        ...
        for row in rows:
            ...
            # 拆分 標案案號/名稱（col[2]：取原始 inner_text 保留換行再分割）
            case_raw_text = await cols[2].inner_text() if len(cols) > 2 else ""
            parts = [p.strip() for p in case_raw_text.split("\n") if p.strip()]
            case_no = parts[0] if parts else ""
            case_name = " ".join(parts[1:]) if len(parts) > 1 else ""
            # 移除案號中可能附帶的 "(更正公告)" 標註
            case_no = re.sub(r"\s*\(更正公告\)\s*$", "", case_no)
    ...
    async def _solve_captcha(self, page: Page) -> bool:
        ...
        while True:
            ...
            if len(b_checkboxes) < 6:
                ...
                refresh_btn = (
                    await page.query_selector("#b_refresh")
                    or await page.query_selector("text=重新整理")
                    or await page.query_selector("text=重新產生")
                )
                if refresh_btn:
    ...
    async def get_award_detail(self, page: Page, tender: dict, max_retries: int = 2) -> dict | None:
        ...
        for attempt in range(1, max_retries + 1):
            result = await self._fetch_and_parse_detail(page, detail_url, case_no, org_name)
            if result is not None:
                return result
            if attempt < max_retries:
                logger.info(f"  詳細頁第 {attempt} 次失敗，重試中...")
                await page.wait_for_timeout(2000)

        logger.warning(f"  詳細頁 {max_retries} 次嘗試皆失敗: {org_name}/{case_no}")
        return None

    async def _fetch_and_parse_detail(self, page: Page, detail_url: str, case_no: str, org_name: str) -> dict | None:
        """單次嘗試載入並解析決標詳細頁"""
        ...
        # 有 CAPTCHA？（擴充偵測關鍵字）
        if any(kw in page_text for kw in ("A區", "撲克牌", "防止惡意程式", "驗證碼", "B區", "poker")):
        ...
        parsed = parse_award_detail(html)
        # 驗證解析結果非空（至少要有基本欄位）
        if not parsed or (not parsed.get("機關名稱") and not parsed.get("標案案號")):
            logger.warning(f"  詳細頁解析結果為空: {org_name}/{case_no}")
            return None
