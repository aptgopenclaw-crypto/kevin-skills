...
def build_summary_html(keyword_counts: dict[str, int], total: int) -> str:
    ...
    # org-only solutions（以機關名稱為 key 計算）
    for solution in ORG_ONLY_SOLUTIONS:
        org_names = SOLUTION_ORG_KEYWORD_MAP.get(solution, [])
        count = sum(keyword_counts.get(org, 0) for org in org_names)
        org_display = ', '.join(f'"{org}"' for org in org_names)
        solution_counts[solution] = {'count': count, 'keywords': org_display}
...
def _build_award_summary_html(keyword_counts: dict[str, int], total: int) -> str:
    """產生決標報表郵件 HTML 內文"""
    today = date.today().strftime("%Y/%m/%d")
    filter_note = f"（採購性質：{FILTER_PROCUREMENT_TYPE}）" if FILTER_PROCUREMENT_TYPE else ""

    solution_counts: dict[str, dict] = {}
    for solution, keywords in SOLUTION_KEYWORD_MAP.items():
        count = sum(keyword_counts.get(kw, 0) for kw in keywords)
        kw_display = ", ".join(f'"{kw}"' for kw in keywords)
        solution_counts[solution] = {"count": count, "keywords": kw_display}
    ...
    for solution, info in sorted(solution_counts.items(), key=lambda x: x[1]["count"], reverse=True):
        ...
        rows_html += (
            f"<tr>"
            f'<td style="padding:6px 12px;border:1px solid #ddd;{bold}color:{color}">{solution}</td>'
            f'<td style="padding:6px 12px;border:1px solid #ddd;{bold}color:{color};font-size:12px">{keywords}</td>'
            f'<td style="padding:6px 12px;border:1px solid #ddd;text-align:right;{bold}color:{color}">{count} 筆</td>'
            f"</tr>"
        )
    ...
    <html>
    <body style="font-family:'Microsoft JhengHei',Arial,sans-serif;color:#333">
        <h2 style="color:#c55a11">🏆 政府採購網決標資料日報</h2>
        <p>查詢日期：<b>{today}</b> {filter_note}</p>
        ...
        <table style="border-collapse:collapse;margin:16px 0">
            <thead>
                <tr style="background:#c55a11;color:#fff">
                    <th style="padding:8px 12px;border:1px solid #ddd;text-align:left">相關產品</th>
                    <th style="padding:8px 12px;border:1px solid #ddd;text-align:left">關鍵字</th>
                    <th style="padding:8px 12px;border:1px solid #ddd;text-align:right">決標筆數</th>
                </tr>
            </thead>
            <tbody>
                <tr style="background:#f5f5f5;font-weight:bold">
...
def send_award_report_email(
    excel_path: str, keyword_counts: dict[str, int], total: int
) -> bool:
    """
    寄送決標報表郵件
    ...
    today = date.today().strftime("%Y/%m/%d")
    subject = f"【決標日報】{today} 政府採購網決標資料 ({total} 筆)"
    ...
    msg.attach(MIMEText(_build_award_summary_html(keyword_counts, total), "html", "utf-8"))
    ...
    try:
        logger.info(f"寄送決標郵件至: {', '.join(MAIL_RECIPIENTS)}")
        with smtplib.SMTP(MAIL_SMTP_HOST, MAIL_SMTP_PORT, timeout=30) as server:
            server.starttls()
            server.login(MAIL_USER, MAIL_PASSWORD)
            server.sendmail(MAIL_USER, MAIL_RECIPIENTS, msg.as_string())
        logger.info("決標郵件寄送成功")
        return True
    except Exception as e:
        logger.error(f"決標郵件寄送失敗: {e}")
        return False
