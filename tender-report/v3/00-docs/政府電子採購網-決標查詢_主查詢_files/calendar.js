...
function(a) {
    ...
    c.check = function() {
        ...
        if ("" !== d.replace(/\s/g, "")) {
            if (!b.test(d)) return c.elem[c.elemv] = "",
            c.msg("日期不符合格式，請重新選擇。"),
            1;
            if (f[0]) return c.elem[c.elemv] = "",
            c.msg("日期不在有效期內，請重新選擇。"),
            1;
            f.value = c.elem[c.elemv].match(b).join(),
            e = f.value.match(/\d+/g),
            ...
            f.auto ? c.creation([e[0], 0 | e[1], 0 | e[2]], 1) : f.value !== c.elem[c.elemv] && (c.elem[c.elemv] = f.value)
        }
    },
    ...
    c.viewDate = function(a, b, d) {
        ...
        f.times = [0 | c.inymd[3] || 0 | c.hmsin[0].value, 0 | c.inymd[4] || 0 | c.hmsin[1].value, 0 | c.inymd[5] || 0 | c.hmsin[2].value],
        c.each(new Array(3),
        function(a) {
    ...
    c.initDate = function() {
        ...
        e = c.inymd = c.elem[c.elemv].match(/\d+/g) || [];
        e.length < 3 && (c.options.start = c.options.start || "", e = c.options.start.match(/\d+/g) || [], e.length < 3 && (e = [d.getFullYear(), d.getMonth() + 1, d.getDate()])),
    ...
    c.view = function(a, f) {
        ...
        c.elemv = /textarea|input/.test(c.elem.tagName.toLocaleLowerCase()) ? "value": "innerHTML",
        ...
        function() {
            ...
            a
        } () + "</div>" + "</div>" + "</div>" + c.viewtb + '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">時間</li>' + '<li><input value="00" readonly>:</li>' + '<li><input value="00" readonly>:</li>' + '<li><input value="00" readonly></li>' + "</ul>" + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>' + '<a id="laydate_today">今天</a>' + '<a id="laydate_ok">確認</a>' + "</div>" + (b.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + "</a>": "") + "</div>", d.body.appendChild(i), c.box = g("#" + h[0]), c.events(), i = null),
        c.follow(c.box),
    ...
    c.parse = function(a, d, e) {
        ...
        return f = f.replace(/MM/, c.digit(a[1])).replace(/DD/, c.digit(a[2])),
        f = f.replace(/hh/, c.digit(0 | d[0])),
        f.replace(/mm/, c.digit(0 | d[1])).replace(/ss/, c.digit(0 | d[2]))
    },
