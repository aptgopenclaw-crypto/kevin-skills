...
from config import (
    ...
    ANTHROPIC_API_KEY, NVIDIA_API_KEY, NVIDIA_VISION_MODEL,
    DEFAULT_VISION_PROVIDER,
)
from detail_parser import parse_detail_page, extract_key_fields
from captcha_solver import solve_captcha_with_anthropic, solve_captcha_with_openai, solve_captcha_with_nvidia

logger = logging.getLogger(__name__)


class TenderScraperV2:
    def __init__(self, headless: bool = False, vision_provider: str = "anthropic"):
        """
        Args:
            headless: 是否無頭模式（建議 False 以便觀察驗證碼）
            vision_provider: 驗證碼辨識用 "anthropic" 或 "openai"
        """
    ...
    async def solve_captcha_and_get_detail(self, page: Page, tender: dict) -> dict | None:
        ...
        if self.vision_provider == "bruteforce":
            # ── 窮舉法：嘗試所有 C(6,2)=15 種組合 ──
            all_combos = list(combinations(range(6), 2))
            combo_idx = 0
            prev_cb_values = None
            max_attempts = max(CAPTCHA_MAX_RETRIES, 20)

            for attempt in range(max_attempts):
                logger.info(f"  窮舉法嘗試 {attempt + 1}/{max_attempts}")
                ...
                # 取得 checkbox values 以偵測驗證碼是否更新
                current_values = []
                for cb in b_checkboxes:
                    val = await cb.get_attribute('value') or await cb.get_attribute('id')
                    current_values.append(val)

                # 若驗證碼更換（不同的牌），重新開始窮舉
                if prev_cb_values is not None and current_values != prev_cb_values:
                    logger.info(f"  偵測到驗證碼更新，重新開始窮舉")
                    combo_idx = 0
                prev_cb_values = current_values

                if combo_idx >= len(all_combos):
                    logger.warning(f"  所有 {len(all_combos)} 種組合已嘗試完畢，重新開始")
                    combo_idx = 0

                combo = all_combos[combo_idx]
                combo_idx += 1

                logger.info(f"  選擇組合 {combo_idx}/{len(all_combos)}: 索引 {combo}")
                ...
                if '機關資料' in new_text or '招標資料' in new_text or '機關代碼' in new_text:
                    logger.info(f"  ✓ 窮舉法驗證碼通過！組合: {combo}，嘗試次數: {attempt + 1}")
                    html = await page.content()
                    return parse_detail_page(html)
            ...
            logger.error(f"  ✗ 窮舉法 {max_attempts} 次嘗試均失敗: {case_no}")
            return None
            ...
            for attempt in range(CAPTCHA_MAX_RETRIES):
                ...
                if self.vision_provider == "anthropic":
                    matches = solve_captcha_with_anthropic(
                        image_bytes=screenshot_bytes,
                        api_key=ANTHROPIC_API_KEY,
                    )
                elif self.vision_provider == "nvidia":
                    ...
                    matches = solve_captcha_with_openai(
                        image_bytes=screenshot_bytes,
                    )
...
async def main():
    ...
    parser.add_argument('--vision', default=DEFAULT_VISION_PROVIDER, choices=['anthropic', 'openai', 'nvidia'], help='Vision AI 提供者')
