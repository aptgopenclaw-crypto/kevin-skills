...
【V3】在 V2 所有優化基礎上，新增 Google Gemini API 原生引擎。

為何不用 Playwright 操作 Gemini 網頁？
  - 違反 Google ToS，帳號可能被封鎖
  - 網頁 DOM 隨時會變，自動化極易失效
  - Google 官方提供 google-generativeai SDK，可直接存取同等模型

新增引擎：
  --ocr-engine gemini   使用 Google Gemini API（gemini-2.0-flash 或 gemini-1.5-pro）
                         與網頁版 Gemini 底層相同，完全合規、穩定、免費額度充足

安裝依賴：
  pip install google-generativeai

取得 API Key：
  https://aistudio.google.com/  → Get API Key（免費，無需信用卡）

用法:
  python make_epub_scanned_v3.py <URL> --ocr-engine gemini --api-key <GEMINI_KEY>
  python make_epub_scanned_v3.py <URL> --ocr-engine gemini --api-key <KEY> --model gemini-1.5-pro
  python make_epub_scanned_v3.py <URL> --ocr-engine gemini --api-key <KEY> --self-correction --strips 4
  python make_epub_scanned_v3.py <URL> --ocr-engine nvidia  --api-key <NVIDIA_KEY>  # 同 V2
"""
import sys
import os
import re
...
def split_into_vertical_strips(image_path: str, n_strips: int = 3,
    ...
    """
    將單頁圖片垂直切為 n_strips 個直排條帶（右→左閱讀順序）。
    """
    if not PIL_AVAILABLE:
        return [image_path]
    try:
...
# ==================== OCR 引擎：Google Gemini API ====================

class GeminiVisionOCRHandler:
    """
    Google Gemini API 原生實作（等同網頁版 Gemini 的模型能力）。

    支援模型：
      gemini-2.0-flash          — 最快，免費額度充足，推薦日常使用
      gemini-1.5-pro             — 最強視覺理解，適合高難度古籍
      gemini-1.5-flash           — 速度與品質平衡

    安裝：pip install google-generativeai
    取得 Key：https://aistudio.google.com/
    """
    is_vision_ai = True
    ...
    def __init__(self, api_key: str, model: str = DEFAULT_MODEL):
        ...
        log(f"Google Gemini API 初始化 (model={model})")

    def _call_api(self, image_path: str, prompt: str) -> str:
        """共用 API 呼叫（直接傳 PIL Image，讓 SDK 處理編碼）。"""
        img = Image.open(image_path)
    ...
    def recognize_text_with_correction(self, image_path: str) -> str:
        ...
        log(f"    🔄 啟動二次驗證校對...")
        try:
            corrected = self._call_api(
                image_path,
                _CORRECTION_PROMPT_TEMPLATE.format(draft=draft)
            )
...
def build_epub_scanned(title: str, author: str,
    ...
    book = epub.EpubBook()
    book.set_identifier(f'taolibrary-v3-{re.sub(r"[^a-zA-Z0-9]", "", title)[:20]}')
...
def process_scanned_book(
    ...
    try:
        if ocr_engine == 'gemini':
            if not api_key:
                log("--api-key 為必填（--ocr-engine gemini）", "ERROR")
                return None
            ocr = GeminiVisionOCRHandler(
                api_key=api_key,
                model=model or GeminiVisionOCRHandler.DEFAULT_MODEL,
            )
        elif ocr_engine == 'nvidia':
            if not api_key:
...
def main():
    parser = argparse.ArgumentParser(
        description='圖片式直排善書 → EPUB3 轉換工具 (V3：支援 Google Gemini API)',
...
範例:
  # Gemini 2.0 Flash（推薦，免費，速度快）
  python make_epub_scanned_v3.py "https://taolibrary.com/..." \\
      --ocr-engine gemini --api-key <GEMINI_KEY>

  # Gemini 1.5 Pro（最強視覺，適合高難度古籍）
  python make_epub_scanned_v3.py "https://..." \\
      --ocr-engine gemini --api-key <KEY> --model gemini-1.5-pro

  # 啟用二次驗證 + 4 條帶（最高精度）
  python make_epub_scanned_v3.py "https://..." \\
      --ocr-engine gemini --api-key <KEY> --self-correction --strips 4

  # NVIDIA 引擎（同 V2 行為）
  python make_epub_scanned_v3.py "https://..." \\
      --ocr-engine nvidia --api-key <NVIDIA_KEY> --model google/gemma-4-31b-it
        '''
    )
    ...
    parser.add_argument('--ocr-engine',
                        choices=['gemini', 'nvidia', 'tesseract', 'paddle'],
                        default='gemini',
                        help='OCR 引擎（預設: gemini）')
    parser.add_argument('--api-key', help='API Key（gemini 或 nvidia 引擎必填）')
    parser.add_argument('--model',
                        default=None,
                        help='模型名稱（gemini 預設: gemini-2.0-flash；'
                             'nvidia 預設: nvidia/llama-3.2-90b-vision-instruct）')
    ...
    parser.add_argument('--end-page', type=int, default=None,
                        help='OCR 到第幾頁為止（預設：處理到最後一頁）')
    parser.add_argument('--max-tokens', type=int, default=600,
                        help='NVIDIA 引擎每次最大輸出 token 數（預設 600）')
